...
fn render_table_interface(t: &TableSchema, names: &TableNameMap) -> String {
    ...
    if t.row_checks.is_empty() {
        return format!("export interface {name} {body}\n");
    }
    // One row-level CHECK = one TS variant union; multiple CHECKs are
    // chained with `&` so TS computes the joint constraint.
    let base = base_interface_name(name, names);
    let mut out = format!("export interface {base} {body}\n");
    let intersections: String = t.row_checks.iter()
        .map(|c| format!(" & ({})", render_row_check_union(c)))
        .collect();
    out += &format!("export type {name} = {base}{intersections};\n");
    out
}

fn render_table_columns(columns: &[swell_analyzer::TableSchemaColumn]) -> String {
...
fn render_row_check_union(check: &TableRowCheck) -> String {
    check.variants.iter()
        .map(|v| {
...
/// Pick an unused `<Name>Base` ident — a sibling table literally named
/// e.g. `<x>_base` would otherwise collide via TS interface merging.
fn base_interface_name(name: &str, names: &TableNameMap) -> String {
