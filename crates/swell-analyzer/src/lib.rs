...
impl Analyzer {
    ...
    pub async fn analyze(&self, sql: &str) -> Result<InferredQuery> {
        ...
        let param_bindings: HashMap<usize, TableColRef> = param_info.iter()
            .enumerate()
        ...
        let columns: Vec<InferredColumn> = described.columns.iter().enumerate()
            .map(|(i, c)| {
                let expr = analyzed.outputs.get(i)
                    .map(|o| &o.expr).unwrap_or(&analyzed::Expr::Unknown);
                let inferred_nullable = decide_nullability(
                    c, &column_meta, build::verdict(expr),
                );
...
pub async fn resolve_column_meta(
    ...
    for row in &rows {
        ...
        out.insert((t as u32, a as i16), ResolvedBaseCol {
            table_ref: TableColRef { schema, table, column }, not_null,
        });
    }
    out
}
...
fn infer_setop_literal_union(expr: &analyzed::Expr) -> Option<String> {
    let analyzed::Expr::SetOp(branches) = expr else { return None };
...
fn build_row_variants(
    ...
    if let Some(v) = build_full_join_variants(analyzed, plan_walk, columns) { return v; }
    if let Some(v) = build_grouping_sets_variants(sql, columns) { return v; }
    Vec::new()
}
...
fn column_ref_name_in_grouping(node: &pg_query::protobuf::Node) -> Option<String> {
    ...
    let NB::ColumnRef(cr) = node.node.as_ref()? else { return None };
...
fn build_grouping_sets_variants(
    ...
    for entry in &sets {
        ...
        match entry.node.as_ref()? {
            NB::List(l) => {
                for item in &l.items {
                    if let Some(name) = column_ref_name_in_grouping(item) { keys.insert(name); }
                }
            }
            NB::ColumnRef(_) => {
                if let Some(name) = column_ref_name_in_grouping(entry) { keys.insert(name); }
            }
    ...
    let variants = variants_keys.iter().map(|keys| {
...
fn decide_nullability(
    ...
    if c.table_oid != 0 && c.attnum > 0 {
        let base_nn = column_meta.get(&(c.table_oid, c.attnum))
