...
fn database_url() -> String {
    std::env::var("DATABASE_URL").expect(
        "property tests require DATABASE_URL pointing at a dev Postgres",
    )
}
...
fn rt() -> &'static Runtime {
    ...
    RT.get_or_init(|| {
        tokio::runtime::Builder::new_current_thread()
            .enable_all().build().expect("build runtime")
    })
}
...
impl PgType {
    ...
    fn expected_ts(&self, enums: &[GenEnum]) -> String {
        match self {
            ...
            PgType::Enum(name) => {
                let e = enums.iter().find(|e| &e.name == name)
                    .expect("enum ref resolves");
                e.labels.iter().map(|l| format!("\"{l}\"")).collect::<Vec<_>>().join(" | ")
            }
        }
    }
}
...
#[derive(Debug, Clone)]
struct GenColumn { name: String, ty: PgType, not_null: bool }

#[derive(Debug, Clone)]
struct GenTable { name: String, columns: Vec<GenColumn> }

#[derive(Debug, Clone)]
struct GenEnum { name: String, labels: Vec<String> }
...
impl GenSchema {
    fn ddl(&self) -> String {
        ...
        out.push_str(&format!("DROP SCHEMA IF EXISTS {} CASCADE;\n", self.namespace));
        out.push_str(&format!("CREATE SCHEMA {};\n", self.namespace));
        for e in &self.enums {
            let labels = e.labels.iter()
                .map(|l| format!("'{l}'")).collect::<Vec<_>>().join(", ");
            out.push_str(&format!("CREATE TYPE {}.{} AS ENUM ({});\n",
...
fn enum_strategy(idx: usize) -> impl Strategy<Value = GenEnum> {
    ...
    proptest::collection::vec(
        ...
        let labels = if labels.len() >= 2 { labels } else { vec!["a".into(), "b".into()] };
        GenEnum { name: name.clone(), labels }
    })
}
...
fn column_strategy(
    idx: usize, enum_names: Vec<String>,
) -> impl Strategy<Value = GenColumn> {
    let name = format!("c_{idx}");
    (type_strategy(enum_names), any::<bool>())
        .prop_map(move |(ty, not_null)| GenColumn {
            name: name.clone(), ty, not_null,
        })
}

fn table_strategy(
    idx: usize, enum_names: Vec<String>,
) -> impl Strategy<Value = GenTable> {
    let name = format!("t_{idx}");
    proptest::collection::vec(0usize..6, 2..=5)
        .prop_flat_map(move |col_idxs| {
            let enum_names = enum_names.clone();
            col_idxs.into_iter().enumerate()
        ...
        .prop_map(move |columns| {
            ...
            let columns: Vec<GenColumn> = columns.into_iter()
            ...
            GenTable {
                ...
                columns: if columns.is_empty() {
                    ...
                    vec![GenColumn { name: "c_0".into(), ty: PgType::Int, not_null: true }]
                } else {
                    columns
                },
            }
...
fn function_strategy(
    idx: usize, enum_names: Vec<String>,
) -> impl Strategy<Value = GenFunction> {
    ...
    (arg_ty, ret_ty).prop_map(move |(a, r)| GenFunction {
        name: name.clone(), arg_type: a, return_type: r,
    })
}
...
fn schema_strategy() -> impl Strategy<Value = GenSchema> {
    ...
    .prop_flat_map(|(enums_seed, table_idxs, fn_idxs)| {
        let enums: Vec<GenEnum> = enums_seed.into_iter().enumerate()
            .map(|(i, e)| GenEnum { name: format!("e_{i}"), ..e }).collect();
        let enum_names: Vec<String> = enums.iter().map(|e| e.name.clone()).collect();
        let tables = table_idxs.into_iter().enumerate()
            .map(|(i, _)| table_strategy(i, enum_names.clone()))
            .collect::<Vec<_>>();
        let functions = fn_idxs.into_iter().enumerate()
...
fn assert_column_matches_schema(
    qname: &str, idx: usize, inferred: &InferredColumn,
    schema_col: &GenColumn, enums: &[GenEnum],
    namespace: &str, table: &str,
) {
    let expected_ts = schema_col.ty.expected_ts(enums);
    assert_eq!(inferred.name, schema_col.name,
        "[{qname}] column {idx} name");
    assert_eq!(inferred.ts_type, expected_ts,
        "[{qname}] column {idx} ts_type — expected `{expected_ts}` for schema type {:?}",
        schema_col.ty);
    assert_eq!(inferred.nullable, !schema_col.not_null,
    ...
    let r = inferred.table_ref.as_ref().unwrap_or_else(||
        panic!("[{qname}] column {idx} `{}` should carry table_ref", schema_col.name));
    assert_eq!(r.schema, namespace, "[{qname}] column {idx} table_ref schema");
    assert_eq!(r.table, table, "[{qname}] column {idx} table_ref table");
    assert_eq!(r.column, schema_col.name, "[{qname}] column {idx} table_ref column");
}

fn assert_param_matches_target(
    qname: &str, idx: usize, inferred: &InferredParam,
    target: &GenColumn, enums: &[GenEnum],
    namespace: &str, table: &str,
) {
    ...
    assert_eq!(inferred.ts_type, expected_ts,
        ...
        idx + 1, target.ty);
    assert_eq!(inferred.nullable, !target.not_null,
        "[{qname}] param ${} nullable should mirror target column NOT NULL (target {}={})",
        idx + 1, target.name, target.not_null);
    let r = inferred.table_ref.as_ref().unwrap_or_else(||
        panic!("[{qname}] param ${} should carry table_ref to {table}.{}",
            idx + 1, target.name));
...
async fn run_one(schema: GenSchema, case_id: u32) {
    ...
    // Apply DDL.
    an.client.simple_query(&schema.ddl())
        .await.unwrap_or_else(|e| panic!("apply DDL: {e}\n--- DDL ---\n{}", schema.ddl()));
    ...
    let _ = an.client.simple_query(
        &format!("DROP SCHEMA IF EXISTS {namespace} CASCADE")
    ).await;
}
...
async fn check_select_star(schema: &GenSchema, table: &GenTable) {
    ...
    assert_eq!(q.columns.len(), table.columns.len(),
    ...
    for (i, (inferred, expected)) in q.columns.iter().zip(&table.columns).enumerate() {
        assert_column_matches_schema(
            "SELECT *", i, inferred, expected, &schema.enums,
...
async fn check_select_subset(schema: &GenSchema, table: &GenTable) {
    ...
    let list = subset.iter().map(|c| c.name.as_str())
        .collect::<Vec<_>>().join(", ");
    let an = analyzer().await;
    ...
    for (i, (inferred, expected)) in q.columns.iter().zip(&subset).enumerate() {
        assert_column_matches_schema(
            "SELECT subset", i, inferred, expected, &schema.enums,
            &schema.namespace, &table.name,
        );
    }
}
...
async fn check_insert_returning(schema: &GenSchema, table: &GenTable) {
    ...
    let col_list = cols.iter().map(|c| c.name.as_str())
        .collect::<Vec<_>>().join(", ");
    let values = (1..=cols.len()).map(|i| format!("${i}"))
        .collect::<Vec<_>>().join(", ");
    let an = analyzer().await;
    let sql = format!(
...
async fn check_update_with_where(schema: &GenSchema, table: &GenTable) {
    ...
    assert_param_matches_target("UPDATE SET", 0, &q.params[0], set_col,
        &schema.enums, &schema.namespace, &table.name);
    // $2 is in WHERE — stays nullable, no table_ref.
    assert_eq!(q.params[1].ts_type, where_col.ty.expected_ts(&schema.enums),
        "UPDATE WHERE: $2 ts_type");
    assert!(q.params[1].nullable,
        "UPDATE WHERE: $2 should stay nullable");
    assert!(q.params[1].table_ref.is_none(),
...
async fn check_left_join(schema: &GenSchema, lhs: &GenTable, rhs: &GenTable) {
    ...
    let Some(rhs_nn) = rhs.columns.iter().find(|c| c.not_null) else { return };
    let an = analyzer().await;
    let sql = format!(
        "SELECT b.{rhs_col} FROM {ns}.{lhs} a LEFT JOIN {ns}.{rhs} b \
         ON a.{lhs_key} = b.{rhs_key}",
        ns = schema.namespace,
        lhs = lhs.name, rhs = rhs.name,
        lhs_key = lhs_key.name, rhs_key = rhs_key.name,
        rhs_col = rhs_nn.name,
    );
...
async fn check_function_call(schema: &GenSchema, f: &GenFunction) {
    ...
    assert_eq!(q.params[0].ts_type, expected_arg,
    ...
    assert_eq!(q.columns[0].ts_type, expected_ret,
