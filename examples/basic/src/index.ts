...
async function main() {
  ...
  const u = await sql("SELECT id, email FROM users WHERE id = $1", userId);

  // 2. Nullable base column.
  const dn = await sql("SELECT display_name FROM users WHERE id = $1", userId);

  // 3. Enum (custom user_role type).
  const r = await sql("SELECT role FROM users WHERE id = $1", userId);

  // 4. count(*) — bigint → string in postgres.js, never null.
  const c = await sql("SELECT count(*) AS n FROM users WHERE org_id = $1", orgId);
