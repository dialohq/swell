...
import { createTypedSql } from "./index.js";
/**
 ...
 */
export async function initStandardPostgres() {
    ...
    const proc = spawn("postgres", ["-D", dataDir, "-k", sockDir, "-h", "", "-c", "fsync=off", "-c", "synchronous_commit=off"], { stdio: "pipe" });
 ...
 */
export function connectExisting(url) {
    const u = new URL(url);
 ...
 */
export async function makeTemplate(opts) {
    ...
    try {
        ...
        const existing = await root.unsafe(`SELECT 1 FROM pg_database WHERE datname = $1`, [name]);
        if (existing.length > 0) {
            const meta = await root.unsafe(`SELECT description FROM pg_shdescription
           JOIN pg_database ON pg_shdescription.objoid = pg_database.oid
           WHERE pg_database.datname = $1`, [name]);
            const recorded = meta[0]?.description;
            ...
            await root.unsafe(`SELECT pg_terminate_backend(pid) FROM pg_stat_activity
    ...
    try {
        await root2.unsafe(`COMMENT ON DATABASE ${quoteIdent(name)} IS ${quoteLit(fingerprint)}`);
    }
    finally {
 ...
 */
export async function withTestDb(opts) {
    ...
    try {
        await root.unsafe(`CREATE DATABASE ${quoteIdent(dbName)} WITH TEMPLATE ${quoteIdent(opts.template)}`);
    }
    finally {
        await root.end({ timeout: 1 });
    }
    ...
    try {
        ...
        try {
            ...
            await cleanup.unsafe(`SELECT pg_terminate_backend(pid) FROM pg_stat_activity
...
// ---------------------------------------------------------------------------
async function resolveMigrationFiles(spec) {
...
function listFiles(dir, ext, recursive) {
    ...
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) {
            if (recursive)
                out.push(...listFiles(full, ext, recursive));
        }
